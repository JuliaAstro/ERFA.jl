@testset "ee00" begin
    ...
    @test ee ≈ -0.8834193235367965479e-5 atol=1e-18
end

@testset "ee00a" begin
    ee = ee00a(2400000.5, 53736.0)
    @test ee ≈ -0.8834192459222588227e-5 atol=1e-18
end

@testset "ee00b" begin
    ee = ee00b(2400000.5, 53736.0)
    @test ee ≈ -0.8835700060003032831e-5 atol=1e-18
end

@testset "ee06a" begin
    ee = ee06a(2400000.5, 53736.0)
    @test ee ≈ -0.8834195072043790156e-5 atol=1e-15
end

@testset "eect00" begin
    ct = eect00(2400000.5, 53736.0)
    @test ct ≈ 0.2046085004885125264e-8 atol=1e-20
end

@testset "eo06a" begin
    eo = eo06a(2400000.5, 53736.0)
    @test eo ≈ -0.1332882371941833644e-2 atol=1e-15
end
...
@testset "eors" begin
    ...
    eo = eors(r, s)
    @test eo ≈ -0.1332882715130744606e-2 atol=1e-15
    @test_throws ArgumentError eors(r[1:2,:], s)
end

@testset "eform" begin
    a, f = eform(ERFA.WGS84)
    @test a ≈ 6378137.0 atol=1e-10
    @test f ≈ 0.0033528106647474807 atol=1e-18
    a, f = eform(ERFA.GRS80)
    @test a ≈ 6378137.0 atol=1e-10
    @test f ≈ 0.0033528106811823189 atol=1e-18
    a, f = eform(ERFA.WGS72)
    @test a ≈ 6378135.0 atol=1e-10
    @test f ≈ 0.0033527794541675049 atol=1e-18
end

@testset "epb" begin
    b = epb(2415019.8135, 30103.18648)
    @test b ≈ 1982.418424159278580 atol=1e-12
end

@testset "epb2jd" begin
    dj0, dj1 = epb2jd(1957.3)
    @test dj0 ≈ 2400000.5 atol=1e-9
    @test dj1 ≈ 35948.1915101513 atol=1e-9
end

@testset "epj" begin
    j = epj(2451545, -7392.5)
    @test j ≈ 1979.760438056125941 atol=1e-12
end

@testset "epj2jd" begin
    dj0, dj1 = epj2jd(1996.8)
    @test dj0 ≈ 2400000.5 atol=1e-9
    @test dj1 ≈ 50375.7 atol=1e-9
end
...
@testset "eqeq94" begin
    ...
    @test ee ≈ 0.5357758254609256894e-4 atol=1e-17
end

@testset "era00" begin
    era = era00(2400000.5, 54388.0)
    @test era ≈ 0.4022837240028158102 atol=1e-12
end

@testset "eceq06" begin
    dr, dd = eceq06(2456165.5, 0.401182685, 5.1, -0.9)
    @test dr ≈ 5.533459733613627767 atol=1e-14
    @test dd ≈ -1.246542932554480576 atol=1e-14
end

@testset "eqec06" begin
    dl, db = eqec06(1234.5, 2440000.5, 1.234, 0.987)
    @test dl ≈ 1.342509918994654619 atol=1e-14
    @test db ≈ 0.5926215259704608132 atol=1e-14
end
